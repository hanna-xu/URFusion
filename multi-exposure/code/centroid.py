...
def parse_args():
    ...
    parser.add_argument('--trainDir', type=str, default='../dataset/train/', help='path to test images')
...
for d in range(len(sub_dirs)):
    train_dataset = SICE_A_eval(args.trainDir+sub_dirs[d], transform=transforms.ToTensor())
    train_loaders.append(DataLoader(train_dataset, batch_size=1, shuffle=False))
...
with torch.no_grad():
    for l in range(len(train_loaders)):
        vectors = []
        for i, sample in enumerate(train_loaders[l]):
        ...
        scipy.io.savemat(args.outputDir+sub_dirs[l]+'.mat', mat_dict)
