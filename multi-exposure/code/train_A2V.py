...
def parse_args():
	...
	parser.add_argument('--device', type=int, default=2, help='device name, cuda:0')
	...
	parser.add_argument('--numEpoch', type=int, default=5)
	parser.add_argument('--patchsize', type=int, default=248)
...
def train(loaders, model_A, model_F, optimizer_A, writer, epoch, num_epochs, begin_time, begin_epoch, device):
	...
	for i, sample in enumerate(loaders['train']):
		...
		source1_batch = sample['img_1'].to(device)
		source2_batch = sample['img_2'].to(device)

		source_batch = torch.cat((source1_batch, source2_batch), dim=0)
		source_batch = source_batch[torch.randperm(source_batch.size(0)), :, :, :]

		for ba in range(source_batch.size(0)):
			random_permutation1 = torch.randperm(source_batch.size(1))
			RGB_shuffle = source_batch[ba:ba+1, random_permutation1, :, :]
		...
		s1_a1, s1_b1, s1_a2, s1_b2, s1_a3, s1_b3, s1_r1, s1_r2 = model_A(source_RGB_shuffle)

		fused_batch, middle = model_F(torch.cat((source_batch, source_batch), 1), s1_a1, s1_b1, s1_a2, s1_b2, s1_a3, s1_b3, s1_r1, s1_r2, modulation = True)

		fused_batch_wo_modulation, _ = model_F(torch.cat((source_batch, source_batch), 1), s1_a1, s1_b1, s1_a2, s1_b2, s1_a3, s1_b3, s1_r1, s1_r2, modulation=False)

		crit_l2 = nn.MSELoss()
		crit_l1 = nn.L1Loss()

		loss_middle = crit_l2(source_batch, middle)
		loss_A2V = crit_l2(source_batch, fused_batch)
		...
		if (i + 1) % 5 == 0:
			print(
				'Appearance: Epoch: [%d/%d], Iter: [%d/%d], Loss: %.5f, Time: ' % (begin_epoch + epoch + 1, num_epochs, i + 1, total_i, loss.item()),
				timeElapsed)
			...
			writer.add_image("source", torchvision.utils.make_grid(source_batch[0:3, :, :, :]), global_step=step)
			writer.add_image("fuse", torchvision.utils.make_grid(fused_batch[0:3, :, :, :]), global_step=step)
			writer.add_image("middle", torchvision.utils.make_grid(middle[0:3, :, :, :]), global_step=step)
