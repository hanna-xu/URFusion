...
def parse_args():
	...
	parser.add_argument('--experiment', default='content-extractor-ir',
	...
	parser.add_argument('--numEpoch', type=int, default=15)
...
def train(loaders, model_R, optimizer_R, writer, epoch, num_epochs, begin_time, begin_epoch, device, **kwargs):
	...
	for i, sample in enumerate(loaders['train']):
		...
		if source1_batch.size(1)<3:
			source1_batch = source1_batch.repeat(1, 3, 1, 1)
		if source2_batch.size(1) < 3:
			source2_batch = source2_batch.repeat(1, 3, 1, 1)
		...
		loss_sim = crit(s1_ex_feas, s2_ex_feas) * 1 + crit(s1_ex_feas_d1, s2_ex_feas_d1) * 4
		...
		if (i + 1) % 5 == 0:
			print('Epoch: [%d/%d], Iter: [%d/%d], Loss: %.5f, Time: ' % (
			epoch + 1 + begin_epoch, num_epochs, i + 1, total_i, loss.item()), timeElapsed)
			...
			writer.add_image("s2_sel_feas_GF", torchvision.utils.make_grid(s2_selected_fea_GF[0:3, p:p + 3, :, :]),
							 global_step=step)
			writer.add_image("s1_ex_feas", torchvision.utils.make_grid(s1_ex_feas[0:3, p:p + 3, :, :]),
							 global_step=step)
			writer.add_image("s2_ex_feas", torchvision.utils.make_grid(s2_ex_feas[0:3, p:p + 3, :, :]),
							 global_step=step)
...
train_dataset = SICE_IR(train_dir, transform=trans_compose)
